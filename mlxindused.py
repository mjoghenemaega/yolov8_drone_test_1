...
# Function to fetch sensor data from ESP01 server
def fetch_temperature():
    try:
        # Make a GET request to the ESP01 server (adjust the IP as needed)
        response = requests.get("http://192.168.10.2")  # Replace with ESP01's IP
        data = response.json()  # Parse JSON response
        
        room_temp = data.get("room_temp", 0.0)
        object_temp = data.get("object_temp", 0.0)
        
        return room_temp, object_temp
...
while running:
    ...
    # Fetch temperature data from ESP01 server
    room_temp, object_temp = fetch_temperature()

    # Create text surfaces for displaying temperature data
    if room_temp is not None and object_temp is not None:
        temp_text = f"Room Temp: {room_temp:.2f}°C, Object Temp: {object_temp:.2f}°C"
    else:
        temp_text = "Error fetching sensor data"
    ...
    # Blit temperature data frame
    window.blit(temp_surface, (50, 800))  # Position the temperature frame at the bottom
    ...
    window.blit(flipped_window, (0, 0))  # Blit the flipped window to the screen
